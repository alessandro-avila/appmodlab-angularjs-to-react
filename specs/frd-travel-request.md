...
> **Stack:** React 19.2.8 · TypeScript 7.0.2 strict · Vite 8.2.1 · react-router 8.3.0 · Zustand 5.0.15 · Zod 4.4.3 · date-fns 4.4.0. Migrated in **Increment 4**.

### Files

| File Path | Role | Lines |
|-----------|------|-------|
| `src/features/travel-request/TravelRequest.tsx` | The screen — JSX, local state, effects | 806 |
| `src/features/travel-request/travel-request-model.ts` | Pure logic — filtering, sorting, derivation, validation | 268 |
| `src/features/travel-request/travel-request-api.ts` | Typed calls through the shared API client | 52 |
...
|---|---|
| `approval-status.directive.js` (`gt-approval-status`) | **deleted, not ported** — zero consumers (Q-10). The badge is a local `statusClass()` reproducing the TEMPLATE's output. |
| `window.confirm()` | `src/components/confirm-dialog.tsx`, still blocking |
| Moment.js | `date-fns` |
| Restangular | `src/lib/api-client.ts` |

### Behaviour notes

Validation is fail-fast and order-dependent — six checks, first failure wins, one message at a time — and **no form library was adopted**, because any library would have had to be configured back into this shape and the configuration would exceed the six `if`s. The unit suite walks the form one field at a time so a reordering fails. The inert search box was revived under ADR-005 / ADR-022.
...
---
## Original Implementation (AngularJS — decommissioned in Increment 4)
