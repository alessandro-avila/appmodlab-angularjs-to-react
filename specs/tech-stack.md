...
| **§13-15** | Assessment summary correction — 34/5/13/13/3 recorded against a measured 41 (5/15/18/3) | A correction to `specs/assessment/modernization.md` and `state.json`, not a stack choice. | housekeeping |
| **§13-16** | **ADR-005 vs ADR-006** on *"both frameworks loaded simultaneously"* | §1.7: *blast radius is the whole plan.* Must be settled before Inc-0. | **before Inc-0** |
...
| **G-1** | **`@playwright/test` is pinned to a pre-release**: `^1.63.0-alpha-2026-07-29`, while `latest` is **1.62.1** and `next` has already moved to `1.63.0-alpha-2026-08-25`. A caret range on a prerelease can float to a newer alpha. | **New finding.** It is baseline harness code, governed by ADR-008 — this phase raises it rather than changing it. §0.6 has just spent real effort making the suite deterministic; a floating alpha undercuts that. The Clock API is present in stable 1.62.1, so pinning exactly or moving to stable are both available. | Inc-0 |
| **G-2** | **Pin `engines.node >= 22.22.0`** in `package.json` | `react-router@8` requires it and the host clears it by two patch versions. Without the pin a contributor on 22.21 gets an opaque install failure. | Inc-0 |
| **G-3** | **Record the named human owner of ADR-015** in `state.json` | An accepted risk whose owner is a role rather than a person is half an owner. See ADR-015 §Gate checklist. | **this gate** |
