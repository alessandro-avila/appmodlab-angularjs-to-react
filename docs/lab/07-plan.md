...
└── adrs/
    ├── adr-005-routing-tanstack-router.md
    ├── adr-006-server-state-tanstack-query.md
    ├── adr-007-client-state-zustand.md
    ├── adr-008-dates-date-fns-explicit-parsing.md    ← the behaviour change
    ├── adr-009-config-vite-env.md
    └── adr-010-auth-jwt-localstorage-accepted-risk.md
```
...
- **ADR-008**, recording that we chose determinism over bug-compatibility
...
> 4. Which ADRs it produced, and whether ADR-008 (date parsing) exists
...
- [ ] ADR-008 exists and references the Gherkin delta
